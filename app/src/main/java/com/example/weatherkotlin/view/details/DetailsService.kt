...
class DetailsService(name: String = "DetailService") : IntentService(name) {
    ...
    private fun loadWeather(lat: String, lon: String) {
        try {
            ...
            try {
                ...
                val weatherDTO: WeatherDTO = Gson().fromJson(getLines(BufferedReader(InputStreamReader(urlConnection.inputStream))), WeatherDTO::class.java)
    ...
    private fun onSuccessResponse(temp: Int?, feelsLike: Int?, condition: String?) {
        ...
        broadcastIntent.putExtra(DETAILS_CONDITIONS_EXTRA, condition)
        LocalBroadcastManager.getInstance(this).sendBroadcast(broadcastIntent)
    }
    ...
    private fun onErrorRequest(error: String) {
        ...
        broadcastIntent.putExtra(DETAILS_REQUEST_MESSAGE_EXTRA, error)
        LocalBroadcastManager.getInstance(this).sendBroadcast(broadcastIntent)
    }
